...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    if ($action == 'terima') {
        ...
        $sql = "UPDATE pendaftar SET status_pembayaran = 'Belum Bayar', bukti_pembayaran = NULL WHERE id_pendaftar = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $id_pendaftar);
        $stmt->execute();
...
$sql = "SELECT p.id_pendaftar, p.no_pendaftaran, p.nama_lengkap, p.status_pembayaran, p.bukti_pembayaran 
        FROM pendaftar p
        ORDER BY FIELD(p.status_pembayaran, 'Menunggu Verifikasi', 'Belum Bayar', 'Lunas'), p.nama_lengkap ASC";
...
<style>
    ...
    .bg-indigo-soft { background-color: rgba(99, 102, 241, 0.1); color: #6366f1; border: 1px solid rgba(99, 102, 241, 0.2); }
    .bg-amber-soft { background-color: rgba(245, 158, 11, 0.1); color: #d97706; border: 1px solid rgba(245, 158, 11, 0.2); }
</style>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        <?php if (empty($pembayaran_list)): ?>
                            <tr><td colspan="7" class="text-center py-5 text-muted">Belum ada data pendaftar.</td></tr>
                        <?php else: ?>
                            <?php $no = 1; foreach ($pembayaran_list as $row): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($row['bukti_pembayaran']): ?>
                                            <a href="<?php echo htmlspecialchars($row['bukti_pembayaran']); ?>" target="_blank" class="btn btn-sm btn-outline-primary rounded-pill">
